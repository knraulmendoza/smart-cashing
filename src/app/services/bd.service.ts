...
export class BdService {
  fechaHoy = `${new Date().getDate()}/${new Date().getMonth() +
  ...
  public consultaId(tabla: string) {
    let obj;
    const lista = this.firestore.collection(tabla);
  ...
  public updateDinerFinal(cobro: iCobro) {
    return this.firestore
      .collection('cobro')
      .doc(cobro.id)
      .update({ dinerFinal: cobro.dinerFinal });
  }
  ...
  public selectWhere(
    ...
    let lista;
    if (key === 1) {
      lista = this.firestore.collection(tabla, ref =>
        ref.where(condicion1, '==', comparar1)
      );
    } else {
      lista = this.firestore.collection(tabla, ref =>
        ref
          .where(condicion1, '==', comparar1)
          .where(condicion2, '==', comparar2)
      );
    }
    const dato = lista.snapshotChanges().map(List => {
      return List.map(dat => {
        obj = dat.payload.doc.data();
        obj.id = dat.payload.doc.id;
        return obj;
